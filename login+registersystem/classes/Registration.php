...
class Registration
{
    ...
    public function __construct()
    {
        ...
        if (isset($_POST["register"])) {
            $this->registerNewUser($_POST['fname'], $_POST['email'], $_POST['pass'], $_POST['cpass']);
        // if we have such a GET request, call the verifyNewUser() method
        } else if (isset($_GET["id"]) && isset($_GET["verification_code"])) {
            if($this->verifyNewUser($_GET["id"], $_GET["verification_code"])){
                header("Location:".$_SERVER['HOST_NAME']."/profile.html");
                exit();
            }
        }
    }
    ...
    private function registerNewUser($user_name, $user_email, $user_password, $user_password_repeat)
    {
        ...
        // finally if all the above checks are ok
            ...
            if (count($result) > 0) {
                ...
                $query_new_user_insert = $this->db_connection->prepare('INSERT INTO users (user_name, password, user_email, user_activation_hash) VALUES(:user_name, :password, :user_email, :user_activation_hash)');
                $query_new_user_insert->bindValue(':user_name', $user_name, PDO::PARAM_STR);
                $query_new_user_insert->bindValue(':password', $user_password_hash, PDO::PARAM_STR);
